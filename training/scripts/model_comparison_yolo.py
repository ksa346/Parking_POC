"""
Standalone YOLO Model Comparison Script
Evaluates YOLO model performance on videos with ground truth car counts.
NO API CALLS - Direct model loading and inference.

Usage:
    python model_comparison_yolo.py --video parking.mp4 --model yolov8n.pt --ground-truth 25
    python model_comparison_yolo.py --video parking.mp4 --model best.pt --ground-truth 30 --trip-wire "{x1=700;y1=300;x2=1600;y2=300}"
...
try:
    ...
    ULTRALYTICS_IMPORT_ERROR = str(sys.exc_info()[1])
...
def parse_trip_wire(trip_wire_spec):
    """Parse text like {x1=700;y1=300;x2=1600;y2=300}."""
...
def parse_class_ids(spec):
    """Parse class IDs from comma-separated text like 2,3,5,7."""
    if not spec.strip():
        return None
    values = []
...
def load_model(model_spec):
    """Load YOLO model from .pt/.onnx or YOLO model name."""
    if not ULTRALYTICS_AVAILABLE:
        raise RuntimeError(
            "ultralytics import failed in the current interpreter.\n"
    ...
    logger.info(f"Loading YOLO model: {model_spec}")
    return YOLO(model_spec)


def detect_frame(model, frame, conf_thresh=0.25, class_ids=None):
    """Run YOLO inference on frame. Returns (boxes, confs)."""
    try:
        ...
        classes = result.boxes.cls.cpu().numpy().astype(int) if result.boxes.cls is not None else np.array([])
        ...
        for i, box in enumerate(boxes_xyxy):
            ...
            if x2 > x1 and y2 > y1:
                ...
                scores.append(float(confs[i]) if i < len(confs) else 0.5)
...
def main():
    parser = argparse.ArgumentParser(
        description="Standalone YOLO model evaluation on video",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  python model_comparison_yolo.py --video video.mp4 --model yolov8n.pt --ground-truth 25\n"
            "  python model_comparison_yolo.py --video video.mp4 --model best.pt --ground-truth 30 --trip-wire \"{x1=700;y1=300;x2=1600;y2=300}\""
        ),
    )

    parser.add_argument("--video", required=True, help="Video file path")
    parser.add_argument("--model", required=True, help="YOLO model path/name (e.g., yolov8n.pt, best.pt)")
    ...
    parser.add_argument("--confidence", type=float, default=25, help="Confidence threshold (0-100, default 25%)")
    ...
    parser.add_argument(
        "--class-ids",
        default="2,3,5,7",
        help="Comma-separated class IDs to include (default COCO vehicle classes: 2,3,5,7)",
    )
    parser.add_argument(
        "--trip-wire",
        default="",
    ...
    try:
        ...
        logger.error(f"Invalid --class-ids: {e}")
        return 1
    ...
    while True:
        ...
        boxes, confs = detect_frame(model, frame, confidence, class_ids)
    ...
    avg_boxes_per_frame = np.mean([f["boxes"] for f in frame_data]) if frame_data else 0.0
    ...
    row = 1
    ws[f"A{row}"] = "YOLO Model Comparison Report"
